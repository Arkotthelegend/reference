...
def generate_english_book(client: AIClient, book, mapping: dict, install: bool, skip_existing: bool) -> None:
    ...
    for ch in mapping["chapters"]:
        ...
        if not text:
            ...
            continue
        _one(client, grade, unit, text, "initial_letter", "english_initial.txt", EN_INITIAL_COUNT, install, skip_existing)
        _one(client, grade, unit, text, "mcq", "english_mcq.txt", EN_MCQ_COUNT, install, skip_existing)


def _one(client, grade, unit, text, kind, prompt_name, target, install, skip_existing) -> None:
    ...
    # Prefer fewer, larger English calls: vocab is global to the unit.
    ...
    write_english(grade, unit, kind, collected, install)
